...
void NMEA_TESTS_ValidateRMC_LogUART(UART_HandleTypeDef *huart) {
    ...
    if (NMEA_ValidateRMC("$GNRMC,080608.000,A") == 0) {
        HAL_UART_Transmit(huart, (uint8_t *)"Test 1 passed\n", 14, 100);
    } else {
        HAL_UART_Transmit(huart, (uint8_t *)"Test 1 failed\n", 14, 100);
    }

    if (NMEA_ValidateRMC("$GPRMC,080608.000,A") == 0) {
        HAL_UART_Transmit(huart, (uint8_t *)"Test 2 passed\n", 14, 100);
    } else {
        HAL_UART_Transmit(huart, (uint8_t *)"Test 2 failed\n", 14, 100);
    }

    if (NMEA_ValidateRMC("$GPGGA,080608.000,A") == -1) {
        HAL_UART_Transmit(huart, (uint8_t *)"Test 3 passed\n", 14, 100);
    } else {
        HAL_UART_Transmit(huart, (uint8_t *)"Test 3 failed\n", 14, 100);
    }

    if (NMEA_ValidateRMC("$G") == -1) {
        HAL_UART_Transmit(huart, (uint8_t *)"Test 4 passed\n", 14, 100);
    } else {
        HAL_UART_Transmit(huart, (uint8_t *)"Test 4 failed\n", 14, 100);
    }
...
void NMEA_TESTS_ParseRMC_LogSTLINK() {
    ...
    printf("%s\n", sentence);
    NMEA_ParseRMC(sentence, &gps_data);

    NMEA_TESTS_LogStructure(&gps_data);

    if (gps_data.time.hours == 8
    		&& gps_data.time.minutes == 6
			&& fabs(gps_data.time.seconds - 8.0) < 0.001
			&& gps_data.fix == 1
    ...
    if (gps_data.time.hours == 18
    		&& gps_data.time.minutes == 56
			&& fabs(gps_data.time.seconds - 9.02) < 0.001
			&& gps_data.fix == 0
			&& fabs(gps_data.latitude - 0) < 0.000002
			&& fabs(gps_data.longitude - 0) < 0.000002) {
    	printf("Test 2 passed\n");
    } else {
    	printf("Test 2 failed\n");
    }
    ...
    if (gps_data.time.hours == 12
    		&& gps_data.time.minutes == 46
			&& fabs(gps_data.time.seconds - 31) < 0.001
			&& gps_data.fix == 1
			&& fabs(gps_data.latitude - 31.999999) < 0.000002
			&& fabs(gps_data.longitude - -71) < 0.000002) {
    	printf("Test 3 passed\n");
    } else {
    	printf("Test 3 failed\n");
    }
    ...
    printf("%s\n", sentence);
    if (NMEA_ParseRMC(sentence, &gps_data) != 0) {
